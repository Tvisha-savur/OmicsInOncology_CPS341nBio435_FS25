...
# Read in the exported graph data
string_network <- read_delim("string_interactions.tsv", delim="\t")

# Print column names to verify correct name for confidence scores
print(colnames(string_network))

# Convert 'combined_score' column to numeric (assuming this is the confidence score column)
string_network <- string_network %>%
  mutate(combined_score = as.numeric(combined_score))

# Filter interactions based on confidence score (keeping only interactions with score ≥ 0.95)
confidence_threshold <- 0.95
string_network_filtered <- string_network %>% filter(combined_score >= confidence_threshold)

# Convert to an igraph object
proteingraph <- graph_from_data_frame(string_network_filtered, directed = FALSE)

# Simplify the graph (removes multiple edges and loops)
proteingraph <- simplify(proteingraph)
