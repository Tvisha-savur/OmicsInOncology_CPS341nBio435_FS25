...
ggplot(t_tests %>% filter(Gene_name %in% c("PSME1", "B2M", "TAPBP")), 
       aes(x = fold_change, fill = Gene_name)) +
  geom_histogram(binwidth = 0.2, alpha = 0.7, position = "identity") +
  theme_minimal() +
  labs(title = "Histogram of Fold Change for Selected Proteins",
       x = "Fold Change",
       y = "Count") +
...
# Filter the expression data for the top 10 significant proteins
heatmap_data <- PDdata %>%
  filter(Gene_name %in% sig_il_genes) %>%  # Use predefined top 10 proteins
  select(Sample_ID, Gene_name, Expression) %>%
  spread(Gene_name, Expression)  # Convert to wide format

# Set Sample_ID as row names and remove the column
...
pheatmap(
  ...
  annotation_col = data.frame(Response = PDdata$Response),  # Add response annotation
  show_rownames = FALSE, 
